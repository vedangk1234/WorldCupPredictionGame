...
// Home shows the ROUND OF 32 knockout matches for logged-in users (the group
// stage lives at /group-stage). Logged-out users are sent to /login by
// requireUser(). The match experience is identical to the group stage —
// MatchCard handles the knockout extra-time / penalty flow when stage='ro32'.
...
export default async function Home() {
  ...
  const { data: matchesData, error: matchErr } = await supabase
    ...
    // Home is the knockout (Round of 32) screen — group fixtures live at
    // /group-stage and must not leak into this list.
    .eq("stage", "ro32")
      ...
      <main className="preds-layout">
        ...
        {!matchErr && matches.length === 0 && (
          <p style={{ color: "var(--chalk-dim)", marginTop: 20 }}>
            No knockout matches yet. Check back once the Round of 32 is set.
          </p>
        )}
