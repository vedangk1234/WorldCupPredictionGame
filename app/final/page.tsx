...
// The Final knockout screen (moved off the home page, which now shows the
// Tournament Stats). Reached from the navbar hamburger menu. The match
// experience is identical to the group stage / RO32 / RO16 / QF / SF / Third —
// MatchCard handles the knockout extra-time / penalty flow when stage='final'.
// Logged-out users → /login. NOTE: the Final and the Third-place match kick off
// at the same time and both stay OPEN simultaneously — each simply lives on its
// own page.
...
export default async function FinalPage() {
  ...
  const { data: matchesData, error: matchErr } = await supabase
    ...
    // This screen is the Final — group fixtures live at /group-stage, the Round of
    // 32 at /ro32, the Round of 16 at /ro16, the Quarter-finals at /qf, the
      ...
      <main className="preds-layout">
        ...
        {!matchErr && matches.length === 0 && (
          <p style={{ color: "var(--chalk-dim)", marginTop: 20 }}>
            No Final yet. Check back once it&apos;s set.
          </p>
        )}
